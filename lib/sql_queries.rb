...
def selects_most_prominent_color_and_returns_with_count
  # "SELECT COUNT(color), name FROM bears GROUP BY color ORDER BY COUNT(name)"
  "SELECT color, COUNT(id) FROM bears GROUP BY color ORDER BY COUNT(color) DESC LIMIT 1 "
end
